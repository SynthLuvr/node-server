...
const findFile = (path, index) => {
	...
	const indexPath = (0, node_path.join)(path, index ?? "index.html");
	return {
		path: indexPath,
...
const createRangeResponse = (c, path, range, size) => {
	...
	return c.body(require_utils_stream.createStreamBody((0, node_fs.createReadStream)(path, {
		start,
		end
	})), 206);
};
const createFileResponse = (c, path, size) => {
	...
	if (!range) {
		c.header("Content-Length", size.toString());
		return c.body(require_utils_stream.createStreamBody((0, node_fs.createReadStream)(path)), 200);
	}
...
const serveStatic = (options = { root: "" }) => {
	...
	return async (c, next) => {
		...
		const found = findFile((0, node_path.join)(root, !optionPath && options.rewriteRequestPath ? options.rewriteRequestPath(filename, c) : filename), options.index);
		if (!found.stats) {
...
const sendFile = async (c, path, options = {}) => {
	...
	const found = findFile((0, node_path.join)(options.root || "", path), options.index);
